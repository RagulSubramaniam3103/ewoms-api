...
export class UserManagementComponent implements OnInit {
  ...
  openImagePopup(user: any, event?: Event): void {
    ...
    this.activePopupUserName = user.userName || user.username || 'User';
    this.activePopupUserEmail = user.email || '';
    const imgStr = user.profileImage || user.ProfileImage;
    
    if (imgStr) {
      this.activePopupImage = 'data:image/webp;base64,' + imgStr;
    } else {
