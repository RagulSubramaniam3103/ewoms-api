...
export class UserPostsComponent implements OnInit, OnChanges {
  ...
  loadStories() {
    ...
    this.authService.getStories(this.currentUserId).subscribe({
      next: (res: any) => {
        ...
        console.log('UserPosts Stories Loaded:', this.stories);
  ...
  openPopup(post: any) {
    if (this.viewMode === 'grid') {
      // PRIVACY CHECK: If blurred, only the owner can open the high-fidelity zoom
      if (post.isBlurred && post.userId !== this.currentUserId) {
        console.warn('Privacy: Access to high-fidelity zoom restricted for this content.');
        return;
      }

      // ONLY open the Amazon Zoom Modal for Grid View (My Contributions)
      this.openImage.emit('data:image/jpeg;base64,' + post.displayImage);
    }
    
    // In list view (Community Feed), clicking the image does nothing as requested.
  }
