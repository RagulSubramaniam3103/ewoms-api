  ...
  loadUserData() {
    ...
    if (token) {
      try {
        ...
        if (!this.profileData.FullName) this.profileData.FullName = decoded.name || decoded['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'] || '';
        if (!this.profileData.UserName) this.profileData.UserName = decoded.unique_name || this.profileData.FullName;
        this.userRole = (decoded.role || decoded['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'] || '').toLowerCase();
        console.log('EditProfile: Detected User Role:', this.userRole);
      } catch (e) {
    ...
    if (this.profileData.UserId) {
      this.authService.getUserById(this.profileData.UserId).subscribe({
        next: (res: any) => {
          ...
          this.profileData.UserName = res.userName || res.UserName || this.profileData.UserName;
